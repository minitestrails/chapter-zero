...
Bridgetown.configure do |config|
  # Preserve dynamically generated Open Graph PNGs under output/og/ (see SiteOgImages generator).
  config.keep_files = (Array(config.keep_files) | ["og"]).uniq

  # Canonical / absolute URLs (Open Graph, Twitter cards, etc.). Use localhost in development
  # so previews and metadata point at your machine. Override with BRIDGETOWN_DEV_URL if needed.
  if Bridgetown.environment.development?
    url ENV.fetch("BRIDGETOWN_DEV_URL", "http://localhost:8000")
  else
  ...
  # Guide collection: ordered chapters (src/_guide/)
  collections do
    guide do
      output true
    ...
    blog do
      ...
      # Bridgetown skips future-dated posts by default. Preview them locally; production hides until publish day.
      future Bridgetown.environment.development?
    end
  end
